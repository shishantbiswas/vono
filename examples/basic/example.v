...
// Or after installing: v install your-username.vono
module main
...
fn main() {
	...
	// HTML response
	app.get('/html', fn (mut c vono.Context) http.Response {
	...
	// Query parameters
	app.get('/search', fn (mut c vono.Context) http.Response {
		query := c.query['q'] or { '' }
		return c.json('{"query": "${query}"}')
	})

	// POST route
	app.post('/api/data', fn (mut c vono.Context) http.Response {
		body := c.body
